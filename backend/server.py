...
def _size_band_easynews(streams: List[Dict[str, Any]], type_: str) -> List[Dict[str, Any]]:
    ...
    Keeps tiny junk encodes away from smallest-first autoplay and big
    "high numbers" out of the picker.  Size unknown → keep."""
    min_gb = 1.0 if type_ == "movie" else 500.0 / 1024.0
    out = []
    for s in streams:
        if isinstance(s, dict) and s.get("_addon_source") == "EASYNEWS":
            gb = s.get("_size_gb")
            if isinstance(gb, (int, float)) and (gb > 5.0 or gb < min_gb):
                continue
        out.append(s)
    return out
...
# ════════════════════════════════════════════════════════════════════
#  (Removed v2.12.0) Admin · YouTube cookies for the Music app
# ════════════════════════════════════════════════════════════════════
# Replaced by the on-device NewPipeExtractor bridge in the Tunes APK.
# The `/admin/music-cookies` HTML page and its two route handlers were
# deleted along with the `music_api._youtube_resolve` backend flow.
# See `music_api.py` header for details.
