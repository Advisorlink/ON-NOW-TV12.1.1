...
# ---------------------------------------------------------------------------
# Xtream VOD matcher (v2.16.42)
# ---------------------------------------------------------------------------
#
# The user's Xtream account already carries a huge on-demand movie
# library (`get_vod_streams`).  Scraped Torrentio / EasyNews links are
# unreliable and can take 5-15 s to resolve to a playable URL — the
# Xtream VOD copy is a direct HTTP mp4/mkv that starts in <1 s.
#
# This endpoint takes a `{title, year}` (and optional `imdb_id`) blob
# from the Vesper Detail page and looks it up in the managed
# provider's VOD library.  A single hit is returned as a direct
# stream URL that the client prepends to its addon-stream list with
# top priority.
#
# The full VOD catalogue is 5 000-15 000 records; we build the index
# ONCE per hour and hold it in a module-level dict for O(1) lookups.
# ---------------------------------------------------------------------------

import re as _re
import asyncio as _asyncio
import base64 as _base64  # noqa: F401 (kept for future series work)

# Reuse the managed provider from instant_bundle so both live-TV and
# VOD hit the same account (single source of truth for creds).
from instant_bundle import _provider_from_env as _managed_provider

# In-memory VOD index: {"expires": epoch, "movies": [Movie, ...]}
# where Movie = {"stream_id": str, "name": str, "year": int|None,
#                "container": str, "tmdb_id": str, "imdb_id": str}
_VOD_INDEX: Dict[str, Any] = {"expires": 0.0, "movies": []}
_VOD_INDEX_LOCK = _asyncio.Lock()
_VOD_TTL_SEC = 60 * 60  # 1 h


def _normalise_title(s: str) -> str:
    """Casefold + strip punctuation + collapse whitespace so
    'Spider-Man: No Way Home' and 'spider man no way home' match."""
    if not s:
        return ""
    s = s.lower()
    # Strip common release-tag noise before normalising (e.g. "[HD]").
    s = _re.sub(r"\[[^\]]*\]|\([^)]*\)", " ", s)
...
def _extract_year_from_name(name: str) -> Optional[int]:
    """Pull a 4-digit 19xx/20xx year out of a VOD entry's display name."""
    if not name:
        return None
...
async def _build_vod_index() -> None:
    """Fetch the full VOD catalogue and normalise it for matching."""
    p = _managed_provider()
    if not p:
        _VOD_INDEX["expires"] = time.time() + 60  # retry in 1 min
        _VOD_INDEX["movies"] = []
        return
    try:
        raw = await _player_api(p, action="get_vod_streams") or []
    except HTTPException as e:
        log.warning("VOD index build failed: %s", e.detail)
        _VOD_INDEX["expires"] = time.time() + 60
        return
    ...
    for m in raw if isinstance(raw, list) else []:
        ...
        if sid is None:
            continue
        # Provider fields are inconsistent — try several sources for year.
        year = None
        for k in ("year", "releaseDate", "release_date"):
            v = m.get(k)
            if v:
                mm = _re.search(r"(19|20)\d{2}", str(v))
                if mm:
                    year = int(mm.group(0))
                    break
        if year is None:
        ...
        movies.append({
            "stream_id": str(sid),
            "name": name,
            "norm": _normalise_title(name),
            # Also index a "name with year stripped" variant so titles
            # like "Dune (2021)" match a user query of just "Dune".
            "norm_no_year": _normalise_title(_re.sub(r"\b(19|20)\d{2}\b", "", name)),
            "year": year,
            "container": str(m.get("container_extension") or "mp4"),
            "tmdb_id": str(m.get("tmdb") or m.get("tmdb_id") or "").strip(),
            "imdb_id": str(m.get("imdb_id") or "").strip(),
        })
    _VOD_INDEX["movies"] = movies
    _VOD_INDEX["expires"] = time.time() + _VOD_TTL_SEC
    log.info("VOD index built: %d movies", len(movies))


async def _ensure_vod_index() -> None:
    if _VOD_INDEX["expires"] > time.time() and _VOD_INDEX["movies"]:
        return
    async with _VOD_INDEX_LOCK:
        if _VOD_INDEX["expires"] > time.time() and _VOD_INDEX["movies"]:
            return
        await _build_vod_index()
...
def _find_vod_match(
    ...
    """Return the best VOD match or None.

    Match priority (stops at first hit):
      1. Exact imdb_id
      2. Exact tmdb_id
      3. Exact normalised title AND same year (±0)
      4. Exact normalised title AND same year (±1) — release-date drift
      5. Exact normalised title with NO year on either side
    """
    movies = _VOD_INDEX["movies"]
    ...
    if imdb_id:
        for m in movies:
            if m["imdb_id"] and m["imdb_id"] == imdb_id:
                return m
    if tmdb_id:
        for m in movies:
            if m["tmdb_id"] and m["tmdb_id"] == tmdb_id:
                return m
    norm = _normalise_title(title)
    if not norm:
        return None
    # Two candidate keys per record ('with year in name' + 'stripped') —
    # we accept either.
    exact_hits = [m for m in movies if norm in (m["norm"], m["norm_no_year"])]
    if not exact_hits:
        return None
    if year is not None:
        # Same year first.
        for m in exact_hits:
            if m["year"] is not None and m["year"] == year:
                return m
        # ±1 year — accounts for provider indexing lag.
        for m in exact_hits:
            if m["year"] is not None and abs(m["year"] - year) <= 1:
                return m
        # Untagged entries (no year on VOD side) are still OK when the
        # user's query has a year — we just can't disambiguate further.
        for m in exact_hits:
            if m["year"] is None:
                return m
        # Query had a year AND every candidate has a DIFFERENT year — we
        # refuse to guess: returning e.g. "Dune (1984)" for a "Dune 2021"
        # query would autoplay the wrong movie without the user noticing.
        return None
    # No year info in the query — return the entry that also has no
    # year, else fall back to the first hit.
    for m in exact_hits:
        if m["year"] is None:
            return m
    return exact_hits[0]
...
@router.post("/vod-match")
async def vod_match(payload: Dict[str, Any] = Body(...)) -> Dict[str, Any]:
    """Match a movie against the managed Xtream provider's VOD library.

    Body:
        {
          "title":   "Dune",
          "year":    2021,        # optional
          "imdb_id": "tt1160419", # optional
          "tmdb_id": "438631",    # optional
          "type":    "movie"      # currently only "movie" is supported
        }

    Returns `{"match": {...}}` on hit — the `url` field is the direct
    Xtream `/movie/USER/PASS/STREAM_ID.EXT` link the client should
    play.  Returns `{"match": null}` when no VOD copy exists so the
    client falls through to its scraped-addon streams.
    """
    title = str(payload.get("title") or "").strip()
    if not title:
        raise HTTPException(400, "title required")
    kind = str(payload.get("type") or "movie").lower()
    if kind != "movie":
        # Series match is a deliberate TODO — needs a get_series_info
        # season/episode traversal that's meaningfully more complex.
        return {"match": None, "reason": "series-not-supported"}
    ...
    await _ensure_vod_index()
    hit = _find_vod_match(title, year, imdb_id=imdb_id, tmdb_id=tmdb_id)
    if not hit:
        return {"match": None}

    p = _managed_provider()
    if not p:
        return {"match": None, "reason": "no-provider"}

    ext = (hit.get("container") or "mp4").strip(".")
    # Xtream VOD URL scheme: /movie/USER/PASS/STREAM_ID.EXT
    scheme = p["scheme"]
    port = p.get("port") or ("443" if scheme == "https" else "80")
    port_part = "" if port in ("80", "443") else f":{port}"
    url = (
        f"{scheme}://{p['host']}{port_part}/movie/"
        f"{p['username']}/{p['password']}/{hit['stream_id']}.{ext}"
    )
    return {
        "match": {
            "url": url,
