 ...
 * full-screen [CrashActivity] instead of the generic "FTA keeps
 * stopping" Android dialog.  Invaluable while we shake out the new
 * native UI on real TV boxes where adb logcat is awkward.
 */
class FtaApp : Application() {
    ...
    override fun onCreate() {
        ...
        Thread.setDefaultUncaughtExceptionHandler { thread, throwable ->
            try {
                Log.e("FtaApp", "Uncaught exception in thread ${thread.name}", throwable)
                ...
                Log.e("FtaApp", "Crash handler itself failed", chain)
